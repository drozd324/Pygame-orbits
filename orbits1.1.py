...
class Particle:
    def __init__(self, x, y, mass):
        ...
        self.mass = mass
        self.radius =  2 #4 * np.log(self.mass + 1) #funny radius so that the circles dont get too big
    ...
    def update(self):
        ...
        #self.radius = 4 * np.log(self.mass + 1)
        if self.sun:
...
def accelerate(p1, p2):
    ...
    dist = np.hypot(x, y)
    force = 0 
    G = .001 #gravitational constant

    if dist < 1:
        force = G * p1.mass * p2.mass
    else:
        force = G * p1.mass * p2.mass / dist**(2/11) # force equation
